...
const MetricsPanel = ({
  ...
  return (
    <>
      {/* --- RIGHT STATS PANEL --- */}
      <div className="absolute top-24 right-8 w-[320px] pointer-events-auto bg-neutral-900 p-5  rounded-2xl border border-white/20 shadow-2xl transition-all duration-300 hover:border-cyan-500/30">
        ...
        <div className="text-white transition-all duration-300 hover:border-cyan-500/30 ">
          <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-2">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-cyan-400">
              Calor Incident En La Placa
            </h3>
...
// ------------------------------------------------------------------
// NEW COMPONENT: 2D INCIDENCE VIEW (PIP)
// ------------------------------------------------------------------
const FocusIncidenceView = ({
  matrixSize,
  focusOffset,
}: {
  matrixSize: number;
  ...
  // Center camera exactly on the current plate height
  ...
  return (
    <div className="absolute bottom-12 right-8 w-[320px] bg-neutral-900 border p-5 border-white/20 rounded-2xl overflow-hidden z-50">
      ...
      <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-2">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-cyan-400">
              Monitor d&apos;Incidència sobre la Placa
            </h3>
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
          </div>

      <Canvas>
        {/* Orthographic Camera for true 2D top-down view */}
        <OrthographicCamera
          makeDefault
        ...
        {/* Simplified Lighting for clear visibility */}
      ...
      >
        ...
        {/* Render the Rays */}
        {/* We use a specific group to ensure rays align with the offset view */}
...
export default function TelescopePage() {
  ...
  const [magicArea, setMagicArea] = useState(236);
      ...
      >
        ...
        >
          ...
          <button
            ...
            className={`w-48 py-3 cursor-pointer rounded-xl font-bold uppercase tracking-wider text-xs transition-all duration-300 shadow-xl border ${
              showRays
