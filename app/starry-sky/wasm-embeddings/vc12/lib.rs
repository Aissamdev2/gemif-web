...
pub fn run_thermal_simulation(
    ...
    let mat_base = Material {
        kt: base_kt,
        emi: base_emi,
    };
    let mat_sink = Material {
        kt: sink_kt,
        emi: sink_emi,
    };
    ...
    // Sink Layers: If sink exists, use the new parameter
    if l_z_sink > 1e-9 {
        // Ensure at least 2 points (top and bottom) if thickness exists
        let points = if n_z_sink < 2 { 2 } else { n_z_sink };
        z_points.extend(linspace(0.0, l_z_sink, points));
    } else {
        z_points.push(0.0);
    }
    ...
    let mut x_edges = vec![0.0; n_xy + 1];
    let mut y_edges = vec![0.0; n_xy + 1];
    let mut z_edges = vec![0.0; nz + 1];

    // interior edges: midpoints
    for i in 1..n_xy {
        x_edges[i] = 0.5 * (x[i - 1] + x[i]);
        y_edges[i] = 0.5 * (y[i - 1] + y[i]);
    }
    // leftmost/rightmost edges
    let dx_first = x[1] - x[0];
    x_edges[0] = x[0] - dx_first / 2.0;
    let dx_last = x[n_xy - 1] - x[n_xy - 2];
    x_edges[n_xy] = x[n_xy - 1] + dx_last / 2.0;

    let dy_first = y[1] - y[0];
    y_edges[0] = y[0] - dy_first / 2.0;
    let dy_last = y[n_xy - 1] - y[n_xy - 2];
    y_edges[n_xy] = y[n_xy - 1] + dy_last / 2.0;

    // Z edges (similar logic)
    for k in 1..nz {
        z_edges[k] = 0.5 * (z[k - 1] + z[k]);
    }
    let dz_first = if nz > 1 { z[1] - z[0] } else { 1e-6 };
    z_edges[0] = z[0] - dz_first / 2.0;
    let dz_last = if nz > 1 { z[nz - 1] - z[nz - 2] } else { 1e-6 };
    z_edges[nz] = z[nz - 1] + dz_last / 2.0;

    // Per-cell widths and area functions
    let mut cell_dx = vec![0.0; n_xy];
    let mut cell_dy = vec![0.0; n_xy];
    let mut cell_dz = vec![0.0; nz];
    for i in 0..n_xy {
        cell_dx[i] = x_edges[i + 1] - x_edges[i];
    }
    for j in 0..n_xy {
        cell_dy[j] = y_edges[j + 1] - y_edges[j];
    }
    for k in 0..nz {
        cell_dz[k] = z_edges[k + 1] - z_edges[k];
    }

    // Area Accessors matching MATLAB's Wy .* Wz logic
    let get_ax = |j_y: usize, k_z: usize| -> f64 {
        // area of face perpendicular to x (dy * dz)
        cell_dy[j_y] * cell_dz[k_z]
    };
    let get_ay = |i_x: usize, k_z: usize| -> f64 {
        // area of face perpendicular to y (dx * dz)
        cell_dx[i_x] * cell_dz[k_z]
    };
    let get_az = |i_x: usize, j_y: usize| -> f64 {
        // top-face area (dx * dy)
        cell_dx[i_x] * cell_dy[j_y]
    };
    ...
    for i in 0..n_xy {
        for j in 0..n_xy {
            ...
            // UPDATED LOGIC: If use_solar_cell is FALSE, q_abs_si acts like a perfect absorber (or just substrate) that produces 0 electricity.
            // But usually, if no solar cell, the material might be just the base or reflector.
            // Assuming "No Solar Cell" means we just treat the 'cell' area as if it were the substrate/base material.

            let q_abs_si = if use_solar_cell {
                q_tot * (1.0 - efficiency) // Normal PV behavior
            } else {
                q_tot // Absorb 100% (turns into heat), 0% electricity
                      // OR: If "No Solar Cell" implies it's just the base plate material there:
                      // q_tot (Base absorbs all)
            };
            ...
            // If use_solar_cell is false, 'cover' regions effectively become simple absorbers
        ...
        for i in 0..n_xy {
            for j in 0..n_xy {
                ...
                if j < n_xy - 1 {
                    ...
                    // symmetry: mirror neighbor => double conductance to j+1
                    add_conn(get_idx(i, j + 1, k), 2.0 * get_ay(i, k), dy);
                }
                ...
                if i < n_xy - 1 {
                    ...
                    add_conn(get_idx(i + 1, j, k), 2.0 * get_ax(j, k), dx);
                }
                // Z Neighbor (k+1)
                if k < nz - 1 {
