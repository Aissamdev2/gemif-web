...
fn eta_cell(concentration_suns: f64) -> f64 {
    ...
    // MATLAB 'log()' es logaritmo natural, en Rust es 'ln()'
    let log_c = concentration_suns.ln();

    // Nueva fórmula: (-0.3396 * ln(C)^2 + 4.3586 * ln(C) + 32.3477) / 100
    let eff = (-0.3396 * log_c.powi(2) + 4.3586 * log_c + 32.3477) / 100.0;
...
pub fn run_thermal_simulation(
    ...
    fin_spacing: f64,
    fin_thickness: f64,
    fin_efficiency_param: f64, // Manual override for fin efficiency (e.g., 0.85)
    optical_efficiency: f64,   // Factor (0.0 - 1.0) for Mirror/Lens losses
    pv_efficiency_param: f64,  // Factor (e.g., 0.20) for standard PV mode
) -> SimulationResult {
    // --- 0. Define Dynamic Materials & Temps ---
    let t_infinity_k = ambient_temp_c + 273.15;
    ...
    let calc_efficiency = |suns: f64| -> f64 {
        if use_standard_pv {
            // Standard PV: Use the user-provided parameter
            pv_efficiency_param
        } else {
    ...
    // Apply Optical Efficiency to the concentrated light collection
    let q_concentrated = magic_area * q_solar * optical_efficiency;
    ...
    let fin_area_multiplier = if effective_use_fins {
        // Use passed parameters for dimensions
        // FIX: Add epsilon to prevent floating point floor error
        ...
        // Use passed fin_efficiency_param
        let a_total_effective = a_base + fin_efficiency_param * a_fins_total;
        ...
        for j in 0..n_vis {
            ...
            for i in 0..n_vis {
                ...
                if is_cpv {
                    ...
                    let q_tot = q_solar * 0.0 + q_incident;
                    let suns = q_tot / 1000.0;
