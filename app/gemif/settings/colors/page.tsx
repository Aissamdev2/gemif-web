...
export default function Page() {
  ...
  const changeSubjectColor = async (_currentState: unknown, formData: FormData) => {
    mutate(process.env.BASE_URL as string + "/api/subjects", await updateSubject(formData))
    return 'Subject updated'
  }

  const changeUserColor = async (_currentState: unknown, formData: FormData) => {
    mutate(process.env.BASE_URL as string + "/api/user", await updateUser(formData))
    return 'User updated'
  }
