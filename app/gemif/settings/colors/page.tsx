...
export default function Page() {
  ...
  const changeUserColor = async (_currentState: unknown, formData: FormData) => {
    ...
    mutate((process.env.NEXT_PUBLIC_BASE_URL as string || process.env.BASE_URL as string) + "/api/user", data)
    return 'User updated'
  }
  ...
  return (
    <div className="h-fit lg:h-full w-full flex bg-white py-3 text-gray-900 font-medium justify-center items-center">
      <div className="w-[95%] h-fit lg:h-[85%] lg:mt-5 flex flex-col grow lg:flex-row gap-10 lg:gap-10 justify-start px-1 lg:px-[60px]">
        <form onSubmit={onSubmitColorForm} className="flex flex-col grow shrink basis-1/2 bg-[#f4f9ff] border border-[#DCEBFF] hover:bg-[#EEF5FF] transition-[background-color] duration-300 rounded-lg p-5">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-row justify-evenly gap-4">
                <div className='flex flex-col items-center'>
                  <label className="flex  items-center mb-1 text-gray-600 text-xs font-medium">Color de texto</label>
          ...
          <div className="flex gap-3 justify-center mt-4">
            ...
            <UpdateButton disabled={!subjects || isLoadingSubjects || !selectedOption || selectedOption === 'Seleccione una asignatura'}/>
          </div>
        </form>
        <form action={dispatchUserColor} className="flex flex-col grow shrink basis-1/2 bg-[#f4f9ff] border border-[#DCEBFF] hover:bg-[#EEF5FF] transition-[background-color] duration-300 rounded-lg p-5">
          <div className="grow flex flex-col gap-3">
          ...
          <div className="flex gap-3 justify-center mt-4">
            ...
            <UpdateButton disabled={!user || isLoadingUser || color == user.color} />
          </div>
        </form>
      </div>
    </div>
...
function UpdateButton({ disabled }: { disabled: boolean}) {
  ...
  return (
    <button aria-disabled={pending || disabled} type="submit" onClick={handleClick} className={`${pending || disabled ? 'opacity-30' : 'opacity-100'} w-full text-center p-1.5 py-2 rounded-md bg-[#4A90E2] text-white text-xs font-medium close-modal-button transition-all duration-300 hover:bg-[#3A7BC4]`}>
      Aplicar
    </button>
  )
}