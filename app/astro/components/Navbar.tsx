      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex justify-between items-center h-20">
          ...
          <Link href="/astro" className="flex items-baseline gap-3 group">
            <span className="font-black text-xl tracking-tighter text-blue-500 transition-transform group-hover:scale-105">
              FESTIVAL GALÀCTIC
            </span>
            <span className={`text-[10px] font-medium tracking-[0.2em] uppercase transition-colors hidden sm:inline ${
              isHome ? 'text-slate-500 group-hover:text-blue-400' : 'text-slate-500 group-hover:text-blue-600'
            }`}>
          ...
          <div className="hidden lg:flex items-center gap-2">
            <div className={`flex items-center p-1 rounded-2xl border ${
              isHome ? 'bg-white/5 border-white/10' : 'bg-slate-500/5 border-slate-200 dark:border-white/5'
            }`}>
                ...
                return (
                  <Link
                    ...
                    className={`relative px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                      isActive
                        ? 'text-blue-500 bg-white/10 shadow-sm'
                        : isHome
                  ...
                  >
                    {section.label}
                    {isActive && (
                      <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-blue-500 rounded-full shadow-[0_0_8px_#3b82f6]"></span>
                    )}
                  </Link>
                );
              })}
              ...
              <Link
                ...
                className={`ml-4 px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest border transition-all ${
                  pathname === fontSection.path
                    ...
                    : isHome
                      ? 'border-white/10 text-slate-300 hover:bg-white hover:text-slate-900 hover:border-white'
