...
import { useWeeklyChallenges } from '@/app/lib/use-weekly-challenges'
...
export default function EditWeeklyChallenge({ id }: { id: string }) {


  const changeWeeklyChallenge = async (_currentState: unknown, formData: FormData) => {
    // Extract multiple choice options from repeated inputs
    if (isMultipleChoice) {
    ...
    const optionsString = options.filter(Boolean).join(','); 
    ...
    // Call API and revalidate SWR cache
    const result = await updateWeeklyChallenge(formData);
  ...
  const challenge = weeklyChallenges?.find((challenge) => challenge.id === id)  
  ...
  useEffect(() => {
    if (challenge && challenge.options) {
      setIsMultipleChoice(challenge.ismultiplechoice)
      setOptions(challenge.options)
      setAnswers(challenge.correctanswers)
      if (challenge.ismultiplechoice)
        setSelectedCorrect(challenge.correctanswers[0])
    }
  }, [challenge])
  ...
  if (error || userError) return <ErrorPage error={error?.message || userError?.message} />

  return (
    <form action={dispatch} className="starting:scale-[0] scale-[1] transition-[transform] duration-300 w-full max-w-3xl h-[90vh] flex flex-col bg-white rounded-xl shadow-xl overflow-hidden">
      <input type="hidden" name="id" value={challenge?.id} />
      <div className="p-4 border-b border-gray-200 shrink-0">
        <h4 className="text-lg font-bold text-gray-900 text-center">Editar desafío</h4>
      </div>
      {errorMessage && (
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
        { isLoading ? (
          ...
          : !weeklyChallenges ? <p className="text-lg font-bold text-gray-900 text-center">No se ha encontrado el desafío</p>
          :
                  ...
                  (user?.role === 'dev' || user?.role === 'admin') && (
                    <div>
                      <label className="flex items-center mb-1 text-gray-600 text-xs font-medium gap-2">
                        ¿Visible?
                        <Toggle name="active" defaultChecked={challenge ? challenge.active : false} />
                      </label>
                    </div>
                  )
                }
                    ...
                    <>
                      ...
                      <div className='flex items-center mb-1 gap-1'>
                        <label htmlFor='strict' className="flex items-center  text-gray-600 text-xs font-medium gap-2">
                          ...
                          <Toggle id='strict' name="strict_answer" defaultChecked={challenge ? challenge.strictanswer : false} />
                        </label>
      ...
      <div className="p-4 border-t border-gray-200 flex items-center justify-end gap-3 shrink-0">
          ...
          <EditButton disabled={!challenge} text={ (user?.role === 'admin' || user?.role === 'dev') && challenge?.suggested ? 'Editar y activar' : 'Editar' } />
        </div>
    </form>
