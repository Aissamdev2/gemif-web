...
import { addEvent, addMainPost, addMessage, updateMessage } from "@/app/lib/actions";
...
export default function EditMessage({ id }: { id: string }) {
  ...
  useEffect(() => {
    if (state === 'Message updated') {
      router.back();
    } else if (state === 'Failed to update message') {
      setErrorMessage('No se pudo actualizar el mensaje');
    }
